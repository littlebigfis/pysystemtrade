# A direct copy of the sysinit/futures/seed_price_data_from_IB.py
# We use this to build a price library by Instrument (not for ongoing price updating)
# Must have IBKR Gateway or TW up and running (need to figure out how to keep gateway up all the time)
# Writes Daily, hourly, and merged Prices into Parquet (hourly data is very limited to the last few months)
# Must manually delete any empty data files otherwise the Generate roll calendars script that comes next will not work (may need to refine this to reduce manual deletes)
